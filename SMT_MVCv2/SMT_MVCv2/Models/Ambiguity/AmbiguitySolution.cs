...
namespace SMT_MVCv2.Models.Ambiguity
{
    public class AmbiguitySolution
    {
        ...
        public string RemoveVerbalAmbiguity(string subject, string initialVerb, string v_tag, int mainSubjectIndex)
        {
            ...
            IdentifyV_root(verb);
            try { solution = verb.Replace(vRoot, verbalAmbiguity_Solution); } catch { solution = verb; }
        ...
        private string removeVerbalAmguity(string subject)
        {
            ...
            if (subject == "I" || subject == "We")
            {
                ...
                else if (verb.Contains("পাওয়া") || verb == "পাওয়া")
                {
                    switch (verb_tag)
                    {
                        case "P_in":
                ...
                else if (verb.Contains("দৌঁড়ানো") || verb == "দৌঁড়ানো")
                {
                    switch (verb_tag)
                    {
                        case "P_in":
                ...
                else if (verb.Contains("আসা") || verb == "আসা")
                {
                    switch (verb_tag)
                    {
                        case "P_in":
        ...
        public string removePrepositionalAmbiguity(DataTable meaningDT, string[] words, string[] postags, string[] objects, string[] object_postags, string subject, string verb, int SentenceLength, int mainSubjectIndex, string PPCase)
        {
            int k = 0;
            int SearchStartIndex = mainSubjectIndex + 1;
            if (words != null)
            {
                TransferAndGeneration_stg aTransferAndGeneration_stg = new TransferAndGeneration_stg();
                for (int i = SearchStartIndex; i < SentenceLength && words[i] != null; i++)
                {
                    string structure = "";
                    if (words[i] == "for")
                    {
                        if (PPCase != null) { structure = PPCase; } else { structure = idntifyPrepositional_Structure(words, i, postags); }
                        switch (structure)
                        {
                            case "case1":
                                prepositionlaAmbiguitySolution += " " + aTransferAndGeneration_stg.IdetifyObjects_NotRelatedWith_Preposition(PStructure_start, PStructure_end, words, meaningDT, subject, verb, mainSubjectIndex);
                                if (words[i + 1] == "him")
                                {
                                ...
                                {
                                     k = 1;
                                    while ((postags[i + k] == "NNP " || postags[i + k] == "NN " || postags[i + k] == "NNS ") && PStructure_end < postags.Length)
                                    {
                                        k++;
                                        if (i + k == postags.Length) { break; }
                                        else if (postags[i + k] == "NNP " || postags[i + k] == "NN " || postags[i + k] == "NNS ") { }
                                        else { k = k - 1; }

                                    }
                                    nounMeaning = nounMeaning_Detection(i, k, words, meaningDT);
                                    if (nounMeaning == "")
                                    {
                                        for (int m = i + 1; m <= k; m++)
                                        {
                            ...
                            case "case2":
                                prepositionlaAmbiguitySolution += " " + aTransferAndGeneration_stg.IdetifyObjects_NotRelatedWith_Preposition(PStructure_start, PStructure_end, words, meaningDT, subject, verb, mainSubjectIndex);
                                nounMeaning = nounMeaning_Detection(i+1, 2, words, meaningDT);
                        ...
                        switch (structure)
                        {
                            case "case1":
                                ...
                                else
                                {
                                    ...
                                    {
                                        k++;
                                        if (i + k == postags.Length) { k = k - 1; break; }
                                        else if (postags[i + k] == "NNP " || postags[i + k] == "NN " || postags[i + k] == "NNS ") { }
                                        else { k = k - 1; }
                            ...
                            case "case2":
                                ...
                                nounMeaning = nounMeaning_Detection(i + 1, k, words, meaningDT);
                                if (nounMeaning == "")
                                {
                                    for (int m = i + 1; m <= k; m++)
                                    {
                    ...
                    {
                        if (PPCase != null) { structure = PPCase; }
                        else { structure = idntifyPrepositional_Structure(words, i, postags); }
                        switch (structure)
                        {
                            case "case1":
                                prepositionlaAmbiguitySolution += " " + aTransferAndGeneration_stg.IdetifyObjects_NotRelatedWith_Preposition(PStructure_start, PStructure_end, words, meaningDT, subject, verb, mainSubjectIndex);
                                ...
                                {
                                    nounMeaning = nounMeaning_Detection(i, 1, words, meaningDT);
                                    if (nounMeaning == "" || postags[i + 1] == "NNP ")
                                    {
                                        nounMeaning = words[i + 1];
        ...
        private string idntifyPrepositional_Structure(string[] objects, int prepositionIndex, string[] object_postags)
        {
            string rule = "";
             //in the morning
             if (objects[prepositionIndex + 1] == "the" && (objects[prepositionIndex + 2] == "morning" || objects[prepositionIndex + 2] == "evening" || objects[prepositionIndex + 2] == "afternoon"))
            {
                PStructure_start = prepositionIndex;
                PStructure_end = prepositionIndex + 2;
                while (object_postags[prepositionIndex + 1] == "NNP " || object_postags[prepositionIndex + 1] == "NN ")
                {
                    PStructure_end++;
                }
                return rule = "case10";
            ...
            else if((objects[prepositionIndex + 1] == "a" || objects[prepositionIndex + 1] == "an" || objects[prepositionIndex + 1] == "the") /*DT=article*/ && (object_postags[prepositionIndex + 2] == "NN " || object_postags[prepositionIndex + 2] == "NNS " || object_postags[prepositionIndex + 2] == "NNP " || object_postags[prepositionIndex + 2] == "NNPS " || object_postags[prepositionIndex + 2] == "PRP " || object_postags[prepositionIndex + 2] == "PRP$ "))
            {
                PStructure_start = prepositionIndex;
                PStructure_end = prepositionIndex + 2;
                while ((object_postags[PStructure_end] == "NNP " || object_postags[PStructure_end] == "NN ") && PStructure_end < object_postags.Length)
                {
                    PStructure_end++;
                    if (PStructure_end >= object_postags.Length - 1) { break; }
                }
                return rule = "case2";
            }
            //in summer
            ...
            //to him,Fahim,me
            else if (((object_postags[prepositionIndex + 1] == "PRP " && objects[prepositionIndex + 1] != "it") || objects[prepositionIndex + 1] == "her" || (object_postags[prepositionIndex + 1] == "NN " || object_postags[prepositionIndex + 1] == "NNS " || object_postags[prepositionIndex + 1] == "NNP " || object_postags[prepositionIndex + 1] == "NNPS " || object_postags[prepositionIndex + 1] == "PRP " || object_postags[prepositionIndex + 1] == "PRP$ ")))
            {

                PStructure_start = prepositionIndex;
                PStructure_end = prepositionIndex;
                while ((object_postags[PStructure_end + 1] == "NNP " || object_postags[PStructure_end + 1] == "NN " || object_postags[PStructure_end + 1] == "NNS ") && PStructure_end < object_postags.Length)
                {
                    PStructure_end++;
                    if (PStructure_end == object_postags.Length - 1) { break; }
                }
        ...
        private string nounMeaning_Detection(int i, int caseFact, string[] words, DataTable meaningDT)
        {
            ...
            string NounPhrase = "";
            int dummi_I = i;
            for (int NounPhrase_Word_Index = i + 1; NounPhrase_Word_Index <= dummi_I + caseFact; NounPhrase_Word_Index++)
            {
                for (int meaning_Word_Index = 0; meaning_Word_Index <= no_of_objects - 1; meaning_Word_Index++)
                {
                    if (NounPhrase_Word_Index > words.Length - 1) { break; }
                    if (words[NounPhrase_Word_Index].ToLower() == meaningDT.Rows[meaning_Word_Index][0].ToString().ToLower())
                    {
                        nounMeaning = meaningDT.Rows[meaning_Word_Index][1].ToString();
                        break;
                    }
                    else
                    {
                        nounMeaning = words[NounPhrase_Word_Index];
                    }
                }

                if (nounMeaning != NounPhrase)
                {
                    NounPhrase += nounMeaning;
                }
            }
            return NounPhrase + " ";
            ...
            //    j++;
            //}
            //if (nounMeaning != "") { return nounMeaning; } else { nounMeaning = words[caseFact+i]; return nounMeaning; }           
        }
        public string NounPhrase_Ambiguity_Solution(DataTable meaningDT, string[] words, string[] postags, string[] objects, string[] object_postags, string subjectEng, string VerbEng, int MainSubjectIndex, string subject)
        {
            int SearchStartIndex = 0;
            string solution = "";
            for (int i = 0; i < MainSubjectIndex; i++)
            {
                if (words[i].ToLower() == "a" || words[i].ToLower() == "an")
                {
                    solution += "একটি ";
                }
                else if (words[i].ToLower() == "the")
                {
                    solution += "টিটি ";
                }
                else if (words[i].ToLower() == "this")
                {
                    solution += "এই টিটি";
                }
                else if (words[i].ToLower() == "this")
                ...
                else if (postags[i] == "IN " || postags[i] == "TO ")
                {
                    solution += removePrepositionalAmbiguity(meaningDT, words, postags, objects, object_postags, subjectEng, VerbEng, SearchStartIndex, MainSubjectIndex + 1, null);
                }


                else
                {
                    nounMeaning = GetMeaning_Objects_Meaning(i, words, meaningDT);
                    if (nounMeaning != "")
                    {
                        solution += " " + GetMeaning_Objects_Meaning(i, words, meaningDT);
                    }
                    else
                    {
                        solution += " " + words[i];
        ...
        private string GetMeaning_Objects_Meaning(int k, string[] WORDS, DataTable MeaningNDT)
        {
            int j = 0;
            ...
            while (j < no_of_objects)
            {
                if (WORDS[k].ToLower() == MeaningNDT.Rows[j][0].ToString().ToLower())
                {
                    nounMeaning = MeaningNDT.Rows[j][1].ToString();
                    break;
                }
                j++;
            }
